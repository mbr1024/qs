...
Page({
  ...
  onShow: function() { this.loadModels(); },
  loadModels: function() {
    var that = this;
  ...
  saveEdit: function() {
    ...
    if (!name) { this.setData({ editingId: -1 }); return; }
    var that = this;
    util.api('/api/shoe-models/' + id, 'PUT', { name: name }).then(function(r) {
      if (r.error) { wx.showToast({ title: r.error, icon: 'none' }); }
      else { wx.showToast({ title: '修改成功', icon: 'success' }); }
  ...
  disableModel: function(e) {
    ...
    wx.showModal({
      title: '确认禁用？',
      content: '禁用后不会在入库/生产/出货中显示',
      success: function(res) {
        if (res.confirm) {
          util.api('/api/shoe-models/' + id + '/disable', 'PUT').then(function() {
  ...
  enableModel: function(e) {
    var id = e.currentTarget.dataset.id;
    var that = this;
    util.api('/api/shoe-models/' + id + '/enable', 'PUT').then(function() {
